...
const Podcast = () => {
    ...
    const handlePlayPodcast = async () => {
        if (podcastEpisodes) {
            ...
            playPodcast(podcastEpisodes[0].episodeUrl)
        }
    }
        ...
        >
            ...
            >
                ...
                {podcastEpisodes && !queueToggle && (
                    ...
                    >
                        <div
                            className={`font-semibold text-xs flex justify-self-end p-1 px-2 ${darkMode ? 'bg-slate-300 text-slate-700' : 'bg-slate-700 text-slate-200'} rounded-md w-fit`}
                        >
                            Latest release
                        </div>
                        <div className="flex-col pt-3 flex">
                            <div className="text-2xl select-none  font-semibold">
                                {podcastEpisodes[0].trackName}
                            </div>
                            ...
                            <div className=" flex select-none items-center gap-1 pb-2">
                                <div
                                    className="hover:scale-115 active:scale-90 hover:cursor-pointer text-blue-600 hover:text-blue-400"
            ...
            <div className="flex-col flex w-11/12">
                {podcastEpisodes &&
                    podcastEpisodes.map((episode, index) => (
                        <PodcastListItem key={index} podcast={episode} />
                    ))}
            </div>
        </div>
    )
